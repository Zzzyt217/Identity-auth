...
/**
 * 风险升级：中风险预警后，若再频繁操作达到阈值，则升级为高风险并拒绝后续查询/认证操作。
 * 使用 Session 记录：是否已中风险预警、预警后操作次数、是否已拒绝。
 */
@Service
public class RiskEscalationService {
    ...
    public boolean isBlocked(HttpSession session) {
        ...
        Object v = session.getAttribute(SESSION_RISK_BLOCKED);
    ...
    /**
     * 清除当前 Session 的风险升级状态（切换账号或登出时调用，避免新用户继承上一用户的高风险/拒绝状态）。
     */
